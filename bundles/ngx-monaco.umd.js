(function (global, factory) {
	...
	(factory((global['ngx-monaco'] = {}),global.ng.core,global.ng.common,global.Rx,global.Rx));
...
var distinctUntilChanged = distinctUntilChanged_1.distinctUntilChanged;
var filter = filter_1.filter;
var map = map_1.map;
var shareReplay = shareReplay_1.shareReplay;
var take = take_1.take;
var takeUntil = takeUntil_1.takeUntil;
var tap = tap_1.tap;
 ...
 * @return {?}
 */
function fromDisposable(source) {
    return Observable.Observable.create(function (observer) {
        var /** @type {?} */ disposable = source(function (e) {
            observer.next(e);
        });
 ...
 */
var COMPLETION_PROVIDERS = new core.InjectionToken('Completion Provider');
/**
 * Use this token to inject the default monaco editor options.
 */
var MONACO_EDITOR_OPTIONS = new core.InjectionToken('Monaco Editor Options');
...
var MonacoEditorService = /** @class */ (function () {
     ...
     * @param {?} zone
     */
    function MonacoEditorService(completionProviders, editorOptions, zone) {
        this.completionProviders = completionProviders;
        this.editorOptions = editorOptions;
        this.zone = zone;
        this.onFileChange = new Subject.Subject();
        this.fileChange$ = this.onFileChange.asObservable();
        this.bootstrap$ = Observable.Observable.create(function (observer) {
    ...
    Object.defineProperty(MonacoEditorService.prototype, "editor", {
        /**
         * @return {?}
         */
        get: function () {
     ...
     * @return {?}
     */
    MonacoEditorService.prototype.setDefaultJavascriptCompilerOptions = function (compilerOptions) {
     ...
     * @return {?}
     */
    MonacoEditorService.prototype.addDefaultJavascriptExtraLib = function (code, filename) {
        monaco.languages.typescript.javascriptDefaults.addExtraLib(code, filename);
    };
    /**
     * @return {?}
     */
    MonacoEditorService.prototype.registerCompletionProviders = function () {
        if (!this.completionProviders) {
            return;
        }
        // Register all the completion providers
        for (var _d = 0, _e = this.completionProviders; _d < _e.length; _d++) {
            var completionProvider = _e[_d];
     ...
     * @return {?}
     */
    MonacoEditorService.prototype.registerModelChangeListener = function (file, model) {
        var _this = this;
        var /** @type {?} */ destroy = fromDisposable(model.onWillDispose.bind(model)).pipe(take(1));
        // Subscribe to changes from the model
        fromDisposable(model.onDidChangeContent.bind(model))
            .pipe(map(function () { return model.getValue(); }), takeUntil(destroy))
            .subscribe(function (content) {
            _this.zone.run(function () {
                _this.onFileChange.next(Object.assign({}, file, { content: content }));
            });
        });
    };
    /**
     ...
     * @return {?}
     */
    MonacoEditorService.prototype.disposeModels = function () {
        if (!window.monaco) {
            return;
        }
        for (var _d = 0, _e = monaco.editor.getModels(); _d < _e.length; _d++) {
            var model = _e[_d];
            model.dispose();
        }
    };
     ...
     * @return {?}
     */
    MonacoEditorService.prototype.load = function (container, options) {
        var _this = this;
        if (options === void 0) { options = {}; }
        var /** @type {?} */ editorOptions = options.editor || this.editorOptions || {};
        return this.bootstrap$.pipe(tap(function () {
            // Dispose all the current models
            _this.disposeModels();
            // Create a new monaco editor
            _this.monacoEditor = monaco.editor.create(container.nativeElement, Object.assign({ theme: options.theme }, editorOptions));
            // Register the completion providers
            _this.registerCompletionProviders();
            // Open the file
            if (_this.file) {
                _this.open(_this.file);
            }
        }));
    };
    /**
     ...
     * @return {?}
     */
    MonacoEditorService.prototype.open = function (file) {
        this.file = file;
        if (!this.monacoEditor) {
            // Exit early if the editor is not bootstrapped yet. It will automatically open the provided file when ready.
            return;
        }
        var /** @type {?} */ uri = monaco.Uri.file(file.uri);
        var /** @type {?} */ model = monaco.editor.getModel(uri);
        if (model) {
     ...
     * @return {?}
     */
    MonacoEditorService.prototype.updateOptions = function (options) {
        if (this.monacoEditor) {
     ...
     * @return {?}
     */
    MonacoEditorService.prototype.setTheme = function (theme) {
        if (window.monaco) {
...
MonacoEditorService.ctorParameters = function () { return [
    { type: Array, decorators: [{ type: core.Optional }, { type: core.Inject, args: [COMPLETION_PROVIDERS,] },] },
    { type: undefined, decorators: [{ type: core.Optional }, { type: core.Inject, args: [MONACO_EDITOR_OPTIONS,] },] },
    { type: core.NgZone, },
]; };
/// <reference path="../../typings/monaco-editor/monaco.d.ts" />
// Services
var MonacoEditorDirective = /** @class */ (function () {
    /**
     * @param {?} monacoEditorService
     * @param {?} editorRef
     */
    function MonacoEditorDirective(monacoEditorService, editorRef) {
        this.monacoEditorService = monacoEditorService;
        this.editorRef = editorRef;
        // Outputs
        this.ready = new core.EventEmitter();
        this.fileChange = new core.EventEmitter();
        this.resize$ = new Subject.Subject();
        this.destroy$ = new Subject.Subject();
    }
    /**
     * @return {?}
     */
    MonacoEditorDirective.prototype.onResize = function () {
        this.resize$.next();
    };
    /**
     * Open the provided file with the editor.
     ...
     * @return {?}
     */
    MonacoEditorDirective.prototype.open = function (file) {
        this.monacoEditorService.open(file);
    };
    /**
     * @return {?}
     */
    MonacoEditorDirective.prototype.ngOnInit = function () {
        var _this = this;
        ...
        this.monacoEditorService.fileChange$.pipe(takeUntil(this.destroy$)).subscribe(function (file) {
            _this.fileChange.emit(file);
        });
        // Resize the editor when the window resizes
        this.resize$.pipe(filter(function () { return Boolean(_this.monacoEditorService.editor); }), map(function () { return ({ width: _this.editorRef.nativeElement.clientWidth, height: _this.editorRef.nativeElement.clientHeight }); }), distinctUntilChanged(function (a, b) { return a.width === b.width && a.height === b.height; }), debounceTime(50), takeUntil(this.destroy$)).subscribe(function (dimension) {
            _this.monacoEditorService.editor.layout(dimension);
        });
    };
    /**
     * @return {?}
     */
    MonacoEditorDirective.prototype.ngOnDestroy = function () {
        this.destroy$.next();
    };
    /**
     * @param {?} changes
     * @return {?}
     */
    MonacoEditorDirective.prototype.ngOnChanges = function (changes) {
        if (changes.options) {
     ...
     * @return {?}
     */
    MonacoEditorDirective.prototype.ngAfterViewChecked = function () {
        this.resize$.next();
    };
 ...
 * @nocollapse
 */
MonacoEditorDirective.ctorParameters = function () { return [
 ...
 * @nocollapse
 */
MonacoEditorModule.ctorParameters = function () { return []; };
...
exports.COMPLETION_PROVIDERS = COMPLETION_PROVIDERS;
exports.MONACO_EDITOR_OPTIONS = MONACO_EDITOR_OPTIONS;

Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=ngx-monaco.umd.js.map
