 ...
 */
const COMPLETION_PROVIDERS = new InjectionToken('Completion Provider');

/**
 * Use this token to inject the default monaco editor options.
 */
const MONACO_EDITOR_OPTIONS = new InjectionToken('Monaco Editor Options');
...
class MonacoEditorService {
     ...
     * @param {?} zone
     */
    constructor(completionProviders, editorOptions, zone) {
        ...
        this.bootstrap$ = Observable$1.create((observer) => {
     ...
     * @return {?}
     */
    setDefaultJavascriptCompilerOptions(compilerOptions) {
     ...
     * @return {?}
     */
    addDefaultJavascriptExtraLib(code, filename) {
     ...
     * @return {?}
     */
    registerModelChangeListener(file, model) {
        ...
        fromDisposable(model.onDidChangeContent.bind(model))
            .pipe(map(() => model.getValue()), takeUntil(destroy))
            .subscribe(content => {
            this.zone.run(() => {
                this.onFileChange.next(Object.assign({}, file, { content }));
            });
        });
    }
    /**
    ...
    load(container, options = {}) {
        ...
        return this.bootstrap$.pipe(tap(() => {
            // Dispose all the current models
            this.disposeModels();
            // Create a new monaco editor
            this.monacoEditor = monaco.editor.create(container.nativeElement, Object.assign({ theme: options.theme }, editorOptions));
...
/// <reference path="../../typings/monaco-editor/monaco.d.ts" />
// Services
class MonacoEditorDirective {
    /**
     * @param {?} monacoEditorService
     * @param {?} editorRef
     */
    constructor(monacoEditorService, editorRef) {
    ...
    ngOnInit() {
        ...
        this.resize$.pipe(filter(() => Boolean(this.monacoEditorService.editor)), map(() => ({ width: this.editorRef.nativeElement.clientWidth, height: this.editorRef.nativeElement.clientHeight })), distinctUntilChanged((a, b) => a.width === b.width && a.height === b.height), debounceTime(50), takeUntil(this.destroy$)).subscribe(dimension => {
...
//# sourceMappingURL=ngx-monaco.js.map
